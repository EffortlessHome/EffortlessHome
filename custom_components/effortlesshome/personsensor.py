...
class eh_personSensor(Entity):
    ...
    def __init__(
        ...
        if not token:
            _LOGGER.warning(
                "[eh_personSensor] Missing token for notification registration."
            )
        else:
            existing = next(
                (
            ...
            if existing:
                _LOGGER.info(
                    "[eh_personSensor] Notification device %s already exists for %s",
                    token,
                    self._email,
                )
            else:
                ...
                _LOGGER.info(
                    "[eh_personSensor] Added notification device for %s: %s",
                    self._email,
                    platform_name,
                )
    ...
    async def async_set_local_tracker(self, entity_id: str):
        ...
        _LOGGER.info(
            "[eh_personSensor] Linked local tracker for %s: %s", self._email, entity_id
        )
    ...
    async def async_set_remote_tracker(self, entity_id: str):
        ...
        _LOGGER.info(
            "[eh_personSensor] Linked remote tracker for %s: %s", self._email, entity_id
        )
        await self.async_update_ha_state()
